...
function PixelSwap({
  ...
  useEffect(() => {
    if (desiredActive === shownActive) {
      if (transition) {
        if (!transition.reversed && animationsRef.current.length > 0) {
          // Reversing back to the shownActive state
          animationsRef.current.forEach(a => {
            if (a.playState !== 'idle') a.reverse();
          });
          setTransition(t => ({ ...t, reversed: true }));
        } else {
          // It's already reversed or no transition, just clean up safely if needed
        }
      }
      return;
    }
    
    if (transition && transition.to === desiredActive) {
      if (transition.reversed && animationsRef.current.length > 0) {
        // We were reversing, but now we want to go forward again!
        animationsRef.current.forEach(a => {
          if (a.playState !== 'idle') a.reverse();
        });
        setTransition(t => ({ ...t, reversed: false }));
      }
      return;
    }

    stopAnimations();
    setTransition({ to: desiredActive, grid: gridRef.current, reversed: false });
  }, [desiredActive, shownActive, transition, stopAnimations]);

  useEffect(() => {
    if (!transition) return;
    if (transition.reversed) return; // Do not rebuild animations if we are just changing direction!

    const settings = configRef.current;
    const { grid: frozenGrid, to } = transition;

    const finish = () => {
      // If we finished going to the target state
      if (!transition.reversed) {
        ...
        // We finished reversing back to the start state!
        stopAnimations();
        setTransition(null); // shownActive remains what it originally was
      }
    };
    ...
    // Create a central promise to track when all animations finish
    const allAnimations = [];
    ...
    frozenGrid.pixels.forEach((pixel, index) => {
      ...
      const anim1 = pixelElement.animate(keyframes.window, timing);
      const anim2 = content.animate(keyframes.content, timing);
      
      animationsRef.current.push(anim1, anim2);
      allAnimations.push(anim1.finished, anim2.finished);
    });

    Promise.all(allAnimations).then(() => {
      // Use a small timeout to ensure React state updates don't conflict with animation frame
      setTimeout(finish, 10);
    }).catch(() => {
      // Animations were cancelled via stopAnimations()
    });

  }, [transition, stopAnimations]);
