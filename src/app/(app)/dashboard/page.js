...
export default function Dashboard() {
  ...
  const totalStyles = hasAccess('master') ? (data.master || []).length : 0;
  const totalSalesQty = hasAccess('sales') ? (data.sales || []).reduce((acc, curr) => acc + Number(curr.qty || 0), 0) : 0;
  const totalFabrics = hasAccess('fabric') ? (data.fabric || []).length : 0;
  const recentOrders = hasAccess('sales') ? (data.sales || []) : [];
  const fulfilmentRate = recentOrders.length && hasAccess('finished') ? Math.round((recentOrders.filter(o => o.status === 'Finished Goods').length / recentOrders.length) * 100) : 0;

  const kpis = [];
  if (hasAccess('master')) kpis.push({ title: "Active Styles", value: totalStyles.toString(), icon: "checkroom", change: "Live" });
  if (hasAccess('sales')) kpis.push({ title: "Total Ordered Qty", value: totalSalesQty.toLocaleString(), icon: "shopping_cart", change: "Units" });
  if (hasAccess('fabric')) kpis.push({ title: "Fabric Types in Stock", value: totalFabrics.toString(), icon: "inventory_2", change: "Live" });
  if (hasAccess('sales') && hasAccess('finished')) kpis.push({ title: "Order Fulfilment", value: `${fulfilmentRate}%`, icon: "task_alt", change: "Calculated" });
  ...
  const totalFabricQty = hasAccess('fabric') ? (data.fabric || []).reduce((acc, curr) => acc + Number(curr.quantity || 0), 0) : 0;
  const totalAccQty = hasAccess('accessories') ? (data.accessories || []).reduce((acc, curr) => acc + Number(curr.quantity || 0), 0) : 0;
  const inventoryData = [];
  if (hasAccess('fabric')) inventoryData.push({ name: 'Fabric', value: totalFabricQty, color: '#D4AF37' });
  if (hasAccess('accessories')) inventoryData.push({ name: 'Accessories', value: totalAccQty, color: '#2A9D8F' });

  // 3. Dynamic Production Bar Chart
  const totalCut = hasAccess('cutting') ? (data.cutting || []).reduce((acc, curr) => acc + Number(curr.cutQty || 0), 0) : 0;
  const totalStitch = hasAccess('stitching') ? (data.stitching || []).reduce((acc, curr) => acc + Number(curr.outputQty || 0), 0) : 0;
  const totalQC = hasAccess('quality') ? (data.quality || []).reduce((acc, curr) => acc + Number(curr.passQty || 0), 0) : 0;
  const totalPacked = hasAccess('packing') ? (data.packing || []).length * 50 : 0; // Approximating based on carton contents for demo

  let barData = [];
  if (comparisonType === 'cut_vs_stitch') {
    barData = [{ name: 'Production', ...(hasAccess('cutting') && { 'Cutting': totalCut }), ...(hasAccess('stitching') && { 'Stitching': totalStitch }) }];
  } else if (comparisonType === 'stitch_vs_qc') {
    barData = [{ name: 'Production', ...(hasAccess('stitching') && { 'Stitching': totalStitch }), ...(hasAccess('quality') && { 'QC Passed': totalQC }) }];
  } else {
    barData = [{ name: 'Production', ...(hasAccess('quality') && { 'QC Passed': totalQC }), ...(hasAccess('packing') && { 'Packed Units': totalPacked }) }];
  }
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="page-content" style={{ padding: '32px' }}>
      ...
      <motion.div variants={itemVariants} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px', marginBottom: '24px' }}>
        {kpis.map((kpi, index) => (
          <div key={index} className="card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', background: 'var(--bg-secondary)', borderRadius: '16px', padding: '24px', border: '1px solid var(--border-color)', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', transition: 'transform 0.3s ease, box-shadow 0.3s ease' }} onMouseOver={(e) => { e.currentTarget.style.transform = 'translateY(-5px)'; e.currentTarget.style.boxShadow = '0 10px 20px rgba(0,0,0,0.1)'; e.currentTarget.style.borderColor = 'rgba(212,175,55,0.3)'; }} onMouseOut={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 6px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = 'var(--border-color)'; }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px', marginBottom: '24px' }}>
        ...
        {hasAccess('sales') && (
          <motion.div variants={itemVariants} className="card" style={{ background: 'var(--bg-secondary)', borderRadius: '16px', padding: '24px', border: '1px solid var(--border-color)', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
        ...
        {(hasAccess('fabric') || hasAccess('accessories')) && (
          <motion.div variants={itemVariants} className="card" style={{ background: 'var(--bg-secondary)', borderRadius: '16px', padding: '24px', border: '1px solid var(--border-color)', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column' }}>
      ...
      {(hasAccess('cutting') || hasAccess('stitching') || hasAccess('quality') || hasAccess('packing')) && (
        <motion.div variants={itemVariants} className="card" style={{ background: 'var(--bg-secondary)', borderRadius: '16px', padding: '24px', border: '1px solid var(--border-color)', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', marginBottom: '48px' }}>
            ...
            <ResponsiveContainer>
              <BarChart data={barData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }} barGap={20} barSize={60}>
                ...
                {comparisonType === 'cut_vs_stitch' && (
                  <>
                    {hasAccess('cutting') && <Bar dataKey="Cutting" fill="#D4AF37" radius={[4, 4, 0, 0]} />}
                    {hasAccess('stitching') && <Bar dataKey="Stitching" fill="#E63946" radius={[4, 4, 0, 0]} />}
                  </>
                )}
                {comparisonType === 'stitch_vs_qc' && (
                  <>
                    {hasAccess('stitching') && <Bar dataKey="Stitching" fill="#E63946" radius={[4, 4, 0, 0]} />}
                    {hasAccess('quality') && <Bar dataKey="QC Passed" fill="#2A9D8F" radius={[4, 4, 0, 0]} />}
                  </>
                )}
                {comparisonType === 'qc_vs_pack' && (
                  <>
                    {hasAccess('quality') && <Bar dataKey="QC Passed" fill="#2A9D8F" radius={[4, 4, 0, 0]} />}
                    {hasAccess('packing') && <Bar dataKey="Packed Units" fill="#457B9D" radius={[4, 4, 0, 0]} />}
                  </>
                )}
              </BarChart>
            </ResponsiveContainer>
