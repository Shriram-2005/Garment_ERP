...
export default function AnalyticsCharts() {
  ...
  const salesByStatus = (data.sales || []).reduce((acc, curr) => {
  ...
  const fabricData = (data.fabric || []).map(f => ({
    name: f.name.substring(0, 10) + '...',
    Quantity: Number(f.quantity || 0)
  }));

  // 4. Costing Analysis (Bar)
  const costingData = (data.costing || []).map(c => ({
    name: c.styleCode,
    Cost: Number(c.totalCost || 0)
  }));

  // 5. Stitching Output (Line)
  const stitchingData = (data.stitching || []).map(s => ({
    name: s.lineNo,
